...
// Helper to remove frontmatter from markdown content
...
// Define paths for the input markdown file, output file, and the scripts folder
const apiInputPath = new URL("../API.md", import.meta.url).pathname;
const apiOutputPath = new URL("../API-GENERATED.md", import.meta.url).pathname;
const scriptsDir = path.dirname(new URL(import.meta.url).pathname);

// Delete the output file if it already exists
if (existsSync(apiOutputPath)) {
  console.log(`Deleting existing file: ${apiOutputPath}`);
  unlinkSync(apiOutputPath);
}

// Read the markdown file
let content = await readFile(apiInputPath, "utf8");
...
// Remove frontmatter from the content before writing to the output file
const contentWithoutFrontmatter = removeFrontmatter(newContent);

// Write to the output file instead of overwriting the input file
await writeFile(apiOutputPath, contentWithoutFrontmatter, "utf8");
console.log(
  `Injection complete: ${markerProcessed} marker scripts and ${regularScriptsProcessed} regular scripts processed.`
);
console.log(`Generated file: ${apiOutputPath}`);
